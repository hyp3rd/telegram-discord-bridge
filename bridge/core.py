...
class Bridge:
            ...
            async for dialog in self.telegram_client.iter_dialogs():
                ...
                for forwarder in config.telegram_forwarders:
                    if forwarder.tg_channel_id in {dialog.name, dialog.entity.id}: # type: ignore
                        ...
                        logger.info("Registered Forwarder %s: Telegram channel '%s' (ID %s) with Discord Channel %s",
    ...
    async def _handle_new_message(self, event):  # pylint: disable=too-many-locals
        ...
        for forwarder in matching_forwarders:
            ...
            if not should_forward_message or forwarder.mention_override:
                ...
                logger.debug("message_forward_hashtags: %s",
                                message_forward_hashtags)

                logger.debug("mention_override: %s",
                                forwarder.mention_override)

                logger.debug("forward_hashtags: %s",
            ...
            message_text = await self.process_message_text(
                message, forwarder.strip_off_links, mention_everyone, mention_roles, config.openai.enabled)
            ...
            if sent_discord_messages:
                ...
                logger.error("Failed to forward TG message %s to Discord",
                            message.id, exc_info=config.application.debug)
    ...
    async def _handle_edit_message(self, event):
        ...
        logger.debug("Editing message id %s from Telegram channel_id: %s", tg_message_id, tg_channel_id)
        ...
        for forwarder in matching_forwarders:
            ...
            if discord_message_id is None:
                logger.debug("No Discord message found for Telegram message %s", tg_message_id)
                continue
            ...
            try:
                ...
                logger.error("Error while editing Discord message: %s", ex, exc_info=config.application.debug)
                return
    ...
    async def _handle_deleted_message(self, event):
        ...
        for forwarder in matching_forwarders:
            ...
            for deleted_id in event.deleted_ids:
                ...
                if discord_message_id is None:
                    logger.debug("No Discord message found for Telegram message %s", deleted_id)
                    continue
                ...
                if discord_channel is None:
                    logger.error("Discord channel %s not found",
                                    forwarder.discord_channel_id)
                    return
                ...
                try:
                    ...
                    logger.debug("Discord message %s not found", discord_message_id, exc_info=config.application.debug)
                    return
                except discord.errors.Forbidden:
                    logger.error("Discord forbade deleting message %s", discord_message_id, exc_info=config.application.debug)
                    return
                except discord.errors.HTTPException as ex:
                    logger.error("Failed deleting message %s: %s", discord_message_id, ex, exc_info=config.application.debug)
                    return
    ...
    async def on_restored_connectivity(self):
        ...
        while True:
            ...
            if config.application.internet_connected and config.telegram.is_healthy is True:
                logger.debug(
                ...
                try:
                    ...
                    logger.error(
                        "Failed to fetch missed messages: %s", exception, exc_info=config.application.debug)
